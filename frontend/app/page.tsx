...
import { 
  Bus, Train, Car, ShieldCheck, MapPin, AlertTriangle, 
  Clock, CheckCircle2, Activity 
} from "lucide-react";

// 1. ADDED TYPES TO FIX 'any' ERRORS
interface Report {
  id: number;
  crowding: string;
  age_minutes: number;
...
export default function BoardWiseDashboard() {
  ...
  // 2. WRAPPED IN useCallback TO FIX useEffect WARNING
  const fetchState = useCallback(async () => {
    try {
      ...
      // 3. USED ERROR VARIABLE TO FIX UNUSED 'e' WARNING
      console.error("API not reachable yet.", error);
    }
  }, []);
  ...
  const simulateTime = async () => {
    await fetch(`http://localhost:8000/api/simulate_time`, { method: "POST" });
    fetchState();
  };

  if (!data) return <div className="h-screen bg-slate-950 text-white flex items-center justify-center">Initializing BoardWise...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-emerald-500/30 pb-20">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden relative shadow-2xl">
            ...
            <div className="p-6">
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 flex gap-3 items-start">
                <div className="bg-indigo-500/20 p-1.5 rounded-md shrink-0"><Activity className="h-5 w-5 text-indigo-400" /></div>
                <div>
          ...
          <div className={`p-6 rounded-2xl border flex items-center justify-between ${isRecommended ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}>
            <div className="flex items-start gap-4">
              {isRecommended ? <CheckCircle2 className="h-8 w-8 text-emerald-400 shrink-0" /> : <AlertTriangle className="h-8 w-8 text-rose-400 shrink-0" />}
              <div>
        ...
        {/* RIGHT COLUMN: Alternatives & Community */}
                ...
                <div key={i} className={`flex items-center justify-between p-3 rounded-xl border ${!isRecommended && i === 0 ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-slate-950 border-slate-800'} transition-all`}>
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-800 p-2 rounded-lg">
                      {alt.icon === 'train' && <Train className="h-5 w-5 text-slate-300" />}
                      {alt.icon === 'bus' && <Bus className="h-5 w-5 text-slate-300" />}
                      {alt.icon === 'car' && <Car className="h-5 w-5 text-slate-300" />}
                    </div>
                    <div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                        <span className="flex items-center gap-1"><Clock className="h-3 w-3"/> {rep.age_minutes}m ago</span>
                        {rep.location_verified && <span className="flex items-center gap-1 text-emerald-500/70"><ShieldCheck className="h-3 w-3"/> Verified Location</span>}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="fixed bottom-6 right-6 w-80 bg-slate-800 border-2 border-indigo-500 rounded-xl shadow-2xl overflow-hidden z-50">
        ...
        <div className="p-4 space-y-3">
          ...
          <button onClick={simulateTime} className="w-full bg-slate-900 hover:bg-slate-700 text-xs py-2 rounded border border-slate-700 flex items-center justify-center gap-2">
            <Clock className="h-3 w-3" /> Simulate +2 Minutes Passing
          </button>
        </div>
      </div>
    </div>
