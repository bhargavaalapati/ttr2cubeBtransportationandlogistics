...
import { FiArrowRight, FiActivity, FiMapPin, FiZap, FiShield } from "react-icons/fi";
      ...
      <nav className="fixed top-0 w-full z-50 bg-slate-950/80 backdrop-blur-lg border-b border-slate-800/60">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-2xl tracking-tight">BoardWise<span className="text-emerald-500">.ai</span></span>
          </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tight leading-[1.1] mb-6">
              The ETA is <span className="text-transparent bg-clip-text bg-linear-to-r from-emerald-400 to-cyan-500">Not Enough.</span>
            </h1>
            {/* FIXED ESCAPED ENTITY */}
            <p className="text-xl text-slate-400 leading-relaxed mb-8 max-w-lg">
              Knowing when a bus arrives doesn&apos;t mean you can board it. BoardWise uses live crowdsourcing and AI to predict urban transit viability before you wait.
            </p>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden transform rotate-2 hover:rotate-0 transition-transform duration-500 hover:shadow-[0_0_50px_-10px_rgba(16,185,129,0.3)]">
              ...
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  <div className="text-emerald-400 font-bold text-xl flex items-center gap-2"><FaBus /> 218D </div>
                ...
                <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-lg">
                  <p className="text-rose-400 font-semibold mb-1">Recommendation</p>
                  <p className="text-slate-300 text-sm">Severe crowding detected. We recommend switching to the Metro (18 mins).</p>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <section id="about" className="py-24 bg-slate-950 border-t border-slate-900">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-5xl font-bold mb-6">Why we built BoardWise.</h2>
            {/* FIXED ESCAPED ENTITY */}
            <p className="text-lg text-slate-400">
              Legacy transit systems treat commuters like dots on a map. They optimize for vehicle tracking, not human outcomes. We are changing the paradigm from &quot;Where is the bus?&quot; to &quot;Can I actually use this bus?&quot;
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: FiMapPin, title: "Ghost Stops", desc: "Buses arrive near the GPS pin, but skip the stop entirely due to traffic or driver behavior." },
              { icon: FiActivity, title: "Blind Overcrowding", desc: "Waiting 15 minutes only to watch a fully-packed bus drive straight past you." },
              { icon: FiZap, title: "Static Routing", desc: "Current apps suggest a route without knowing if the transit medium is currently viable." }
            ].map((feature, i) => (
              <motion.div 
                key={i}
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <p className="text-lg text-slate-400 mb-8">
                BoardWise doesn&apos;t just display data; it calculates a deterministic Boarding Confidence Score (BCS) in real-time.
              </p>
              
              <div className="space-y-6">
                {[
                  { title: "Time-Decayed Reports", desc: "User reports decay exponentially. A report from 2 minutes ago overrides a report from 20 minutes ago." },
                  { title: "Location-Verified Trust", desc: "Reports authenticated via hardware GPS proximity get a 1.5x weight multiplier." },
                  { title: "Multi-Modal Failover", desc: "If a bus BCS drops below 40%, the engine instantly routes you to the Metro or an Auto." }
                ].map((item, i) => (
            ...
            >
              <h4 className="text-center text-slate-400 font-bold tracking-widest uppercase mb-8">BCS Formula Breakdown</h4>
              <div className="space-y-4">
                <StatBar label="Crowding Dynamics" percent="40%" color="bg-indigo-500" />
                <StatBar label="Stop Reliability History" percent="30%" color="bg-emerald-500" />
                <StatBar label="Historical Punctuality" percent="20%" color="bg-cyan-500" />
                <StatBar label="Data Freshness Multiplier" percent="10%" color="bg-amber-500" />
              </div>
      ...
      <footer className="bg-slate-950 border-t border-slate-900 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl tracking-tight text-slate-400">BoardWise<span className="text-emerald-700">.ai</span></span>
          </div>
...
function StatBar({ label, percent, color }: { label: string, percent: string, color: string }) {
  return (
    <div>
